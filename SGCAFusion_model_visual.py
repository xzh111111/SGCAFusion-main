...
class BasicLayer(nn.Module):
    ...
    def forward(self, x_ivif, x_mfif, trainingTag, return_features=False):
        """
        return_features=False:
            Keep the original behaviour.
            trainingTag=1 returns IVIF branch output.
            trainingTag=2 returns MFIF branch output.

        return_features=True:
            Return three tensors:
            selected_output, ivif_branch_feature, mfif_branch_feature.
        """
        global spe_transformer_cur_depth

        if trainingTag == 1:
            # IVIF task branch is the selected branch.
            ...
            selected_output = x_ivif

        elif trainingTag == 2:
            # MFIF task branch is the selected branch.
            for i, blk in enumerate(self.blocks):
            ...
            selected_output = x_mfif

        else:
            raise ValueError("trainingTag must be 1 for IVIF or 2 for MFIF.")

        if return_features:
            return selected_output, x_ivif, x_mfif
...
class TransformerNet(nn.Module):
    def __init__(self):
        ...
        self.patch_unembed = PatchUnEmbed(
            patch_size=1, out_chans=embed_dims[0], embed_dim=embed_dims[0], kernel_size=3)
    ...
    def _unembed_and_crop(self, x, mod_pad_w, mod_pad_h):
    ...
    def forward(self, x_ivif, x_mfif, trainingTag, return_features=False):
        ...
        if return_features:
            x_selected, x_ivif_branch, x_mfif_branch = self.layer1(
                x_ivif, x_mfif, trainingTag, return_features=True
            )

            x_selected = self._unembed_and_crop(
                x_selected, mod_pad_w_ivif, mod_pad_h_ivif
            )
            x_ivif_branch = self._unembed_and_crop(
                x_ivif_branch, mod_pad_w_ivif, mod_pad_h_ivif
            )
            x_mfif_branch = self._unembed_and_crop(
                x_mfif_branch, mod_pad_w_mfif, mod_pad_h_mfif
            )

            return x_selected, x_ivif_branch, x_mfif_branch

        x = self.layer1(x_ivif, x_mfif, trainingTag, return_features=False)
        x = self._unembed_and_crop(x, mod_pad_w_ivif, mod_pad_h_ivif)
...
class TransformerSpecificExtractor(nn.Module):
    ...
    def forward(self, x_ivif, x_mfif, trainingTag, return_features=False):
        x = self.SwinTransformerSpecific(
            x_ivif, x_mfif, trainingTag, return_features=return_features
        )
        return x
...
class TwoBranchesFusionNet(nn.Module):
    def __init__(self, s, n, channel, stride):
        ...
        # Keep your current ablation decoder setting.
    ...
    def forward_MultiTask_branch(self,
                                 fea_com_ivif,
                                 fea_com_mfif,
                                 trainingTag=2,
                                 return_features=False):
        """
        return_features=False:
            Original behaviour. Return only selected branch output.

        return_features=True:
            Return:
                fea_fused: selected task-specific output used by decoder
                fea_ivif:  IVIF branch feature
                fea_mfif:  MFIF branch feature
        """
        if return_features:
            fea_fused, fea_ivif, fea_mfif = self.extractor_multask(
                fea_com_ivif, fea_com_mfif, trainingTag, return_features=True
            )
            return fea_fused, fea_ivif, fea_mfif

        x = self.extractor_multask(
            fea_com_ivif, fea_com_mfif, trainingTag, return_features=False
        )
        return x
